...
/*   Updated: 2024/01/11 12:41:50 by nromito          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*str[4026];
	char		*buf;
	char		*out;

	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, 0, 0) < 0 || fd > 4026)
	{
		free (str[fd]);
		str[fd] = NULL;
		return (NULL);
	...
	if (!buf)
		return (NULL);
	str[fd] = read_line(str[fd], buf, fd);
	free (buf);
	buf = NULL;
	out = steal_line(str[fd]);
	str[fd] = clean_the_static_and_steal(str[fd]);
	return (out);
}
